...
pub static UNSUPPORTED_EVENTS: [&str; 3] = ["READY", "RESUMED", "UNKNOWN"];
...
pub async fn cache_and_publish(json: Value) -> Result<Response, RedisError> {
    ...
    let counter = meter.u64_counter("gateway.events.total").build();

    counter.add(1, &[opentelemetry::KeyValue::new(
        "event",
        event_name.clone()
    )]);
    ...
    // ? cloning here so i don't have to deal with the lifetime issues of the
    // guild create event futures
...
async fn is_duplicate(json: &Value) -> Result<bool, RedisError> {
    let hash = {
    ...
    let response: Option<bool> = redis()
        .set(
            format!(
                "discord:event:{}:{}",
...
async fn cache(
    ...
    match event_name.as_str() {
        "MESSAGE_CREATE" | "MESSAGE_UPDATE" => {
            key = format!(
                "discord:message:{}",
                json["d"]["id"].as_str().expect("message id not found")
            );
            ...
            let author_id = json["d"]["author"]["id"]
                .as_str()
            ...
            data = {
                //? redact important fields for privacy
        ...
        "GUILD_CREATE" | "GUILD_UPDATE" => {
            key = format!(
                "discord:guild:{}",
                json["d"]["id"].as_str().expect("guild id not found")
            );
        ...
        "GUILD_DELETE" => {
            ...
            let mut delete = vec![format!(
                "discord:guild:{}",
                json["d"]["id"].as_str().expect("guild id not found")
            )];

            let channels: Option<Vec<String>> = redis()
                .smembers(format!(
            ...
            if let Some(channels) = channels {
                delete.extend(
                    channels
                        .iter()
                        .map(|channel| format!("discord:channel:{channel}"))
                );
            }

            let emojis: Option<Vec<String>> = redis()
                .smembers(format!(
            ...
            if let Some(emojis) = emojis {
                delete.extend(
                    emojis.iter().map(|emoji| format!("discord:emoji:{emoji}"))
                );
            }

            let members: Option<Vec<String>> = redis()
                .smembers(format!(
            ...
            let roles: Option<Vec<String>> = redis()
                .smembers(format!(
                    "discord:guild:{}:roles",
                    json["d"]["id"].as_str().expect("guild id not found")
                ))
            ...
            if let Some(roles) = roles {
                delete.extend(
                    roles.iter().map(|role| format!("discord:role:{role}"))
                );
            }
        ...
        "GUILD_ROLE_CREATE" | "GUILD_ROLE_UPDATE" => {
            key = format!(
                "discord:role:{}",
                json["d"]["role"]["id"].as_str().expect("role id not found")
            );
        ...
        "GUILD_ROLE_DELETE" => {
            key = format!(
                "discord:role:{}",
                json["d"]["role_id"].as_str().expect("role id not found")
            );
        ...
        "GUILD_EMOJIS_UPDATE" => {
            let emojis: Option<Vec<String>> = redis()
                .smembers(format!(
                    "discord:guild:{}:emojis",
                    json["d"]["guild_id"].as_str().expect("guild id not found")
                ))
                .await?;

            if let Some(emojis) = emojis {
                let mut emoji_shards: Vec<Vec<String>> =
                    vec![Vec::new(); EMOJI_SHARDS];
                ...
                for (shard_id, emojis) in emoji_shards.into_iter().enumerate() {
                    if !emojis.is_empty() {
                        let _: () = pipeline
                            .srem(format!("discord_emojis:{shard_id}"), emojis)
                            .await?;
                    }
                }
            }
...

            {
                ...
                for (shard_id, emojis) in emoji_shards.into_iter().enumerate() {
                    if !emojis.is_empty() {
                        let _: () = pipeline
                            .sadd(format!("discord_emojis:{shard_id}"), emojis)
                            .await?;
                    }
                }
            }
            ...
            let _: () = pipeline
                .sadd(
                    ...
                    json["d"]["emojis"]
                        ...
                        .iter()
                        .map(|emoji| {
                            emoji["id"].as_str().expect("emoji id not found")
                        })
                        .collect::<Vec<&str>>()
                )
                .await?;
        ...
        "THREAD_UPDATE" => {
            key = format!(
                "discord:channel:{}",
                json["d"]["id"].as_str().expect("channel id not found")
            );
        ...
        "GUILD_MEMBER_UPDATE" => {
            update_member(pipeline, &mut json["d"].clone()).await?;
    ...
    let cached: Option<Value> = redis()
        .json_get(&key, None::<String>, None::<String>, None::<String>, "$")
        .await?;
    ...
    if let Some(expire) = expire {
        let _: () = pipeline.expire(&key, expire, None).await?;
    }
...
fn json_to_cache_model(data: &Value, meta: Option<&Value>) -> Value {
    json!({
        "data": data,
...
fn json_merge(base: &Value, update: &Value) -> Value {
    match (base, update) {
...
async fn update_user(
    ...
    let key = format!(
        "discord:user:{}",
        json["id"].as_str().expect("user id not found")
    );

    let cached: Option<Value> = redis()
        .json_get(&key, None::<String>, None::<String>, None::<String>, "data")
        .await?;

    match cached {
        Some(cached) => {
            let _: () = pipeline
                .json_set(&key, "data", json_merge(&cached, json), None)
                .await?;
        }
        None => {
            let _: () = pipeline
                .json_set(&key, "$", json_to_cache_model(json, None), None)
                .await?;
        }
    }

    let _: () = pipeline.expire(&key, 86400, None).await?;
...
async fn update_member(
    ...
    let key = format!(
        "discord:member:{}:{}",
        json["guild_id"].as_str().expect("guild id not found"),
        user_id
    );
    ...
    if user_id != APPLICATION_ID.as_str() {
        let _: () = pipeline.expire(&key, 600, None).await?;
    }
