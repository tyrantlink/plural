use std::{collections::HashMap, env, io::Read, str, sync::Arc};
...
lazy_static! {
    ...
    static ref PATH_PATTERNS: HashMap<&'static str, Regex> = [
        ...
        ("/interactions/:id/:token/callback",
            Regex::new(r"^/interactions/\d+/[a-zA-Z0-9_-]+/callback$"
        ).unwrap()),
        ("/webhooks/:id/:token",
            Regex::new(r"^/webhooks/\d+/[a-zA-Z0-9_-]+$").unwrap()),
        ("/webhooks/:id/:token/messages/:id",
            Regex::new(
                r"^/webhooks/\d+/[a-zA-Z0-9_-]+/messages/(\d+|@original)$"
        ).unwrap()),
        ...
        ("/webhooks/:id/:token",
            Regex::new(r"^/webhooks/\d+/[a-zA-Z0-9_-]+$").unwrap()),
        ("/webhooks/:id/:token/:messages/:id",
            Regex::new(r"^/webhooks/\d+/[a-zA-Z0-9_-]+/messages/\d+$"
        ).unwrap()),
...
async fn proxy_handler(
    ...
    let path_key = (string_method.clone(), path.clone());

    let bucket = state.bucket_path_map.read().get(&path_key).cloned();

    let rate_limit = {
        let mut rate_limits = state.rate_limits.write();

        let token_buckets = rate_limits.entry(token.clone()).or_default();
    ...
    let method =
        Method::from_bytes(request.method().as_str().as_bytes()).unwrap();
    let url = format!(
        "{}{}",
        DISCORD_API_URL,
    ...
    loop {
        ...
        match client_req.send().await {
            Ok(res) => {
                ...
                let _b64_token = BASE64_STANDARD_NO_PAD
                    .decode(
                        token
                            .split('.')
                            .next()
                ...
                if bucket.is_some() {
                    ...
                    let remaining =
                        remaining.unwrap_or("1").parse().unwrap_or(1);

                    let reset_time = Instant::now() +
                        Duration::from_secs(
                            reset.unwrap_or("0").parse().unwrap_or(0)
                        );
                ...
                if trace {
                    if let (Some(limit), Some(remaining)) = (limit, remaining) {
                        cx.span().update_name(format!(
                            "{method} {formatted_path} {remaining}/{limit}"
                        ));
                    }
                ...
                if !res.status().is_success() && trace {
                    body_bytes =
                        res.bytes().await.unwrap_or_else(|_| Bytes::new());
